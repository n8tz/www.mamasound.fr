...
export function service( server ) {
	...
	server.use(function loop( req, res, _next ) {
		...
		if ( req.query.cache ) {
		...
		else if (false&& req.query.w || req.query.h ) {
			...
			return staticServe(req, res, function () {
				...
				try {
					found = fs.statSync(upDir + asked);
					console.log('resize : ', upDir + asked, found);
					if ( !found )
						return res.send(503);
					sharp(upDir + asked)
						.on('error', err => {
							console.log('error', upDir + asked, err)
							res.send(503);
						})
						...
						.toBuffer(( err, buffer ) => {
							try {
								...
								console.log('resize failed : ', upDir + targetWebpFn, e);
								res.send(500);
							}
						});
				} catch ( e ) {
